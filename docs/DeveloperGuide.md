...
5. **Improve email validation to detect invalid domains:** The current implementation accepts email inputs without a period `.` in the domain (e.g. john@gmailcom), which is not a valid email format. We plan to improve the email validation to ensure that the domain contains at least one period, preventing incorrect user inputs. 
